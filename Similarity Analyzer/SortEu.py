...
import os
import csv
filename='Euclidean_Similar_Articles.csv'
name='Euclidean Similarity with Article #1'

###---This block of code sorts the values in Ascending order!---###
with open('mergedfile.csv') as fi, open(filename, "w") as fo:
    data=csv.reader(fi)
    csv_writer = csv.writer(fo)
    csv_writer.writerows(sorted(data, key=lambda x:float(x[0])))
